...
describe('FIQL parser', function () {
	...
	it('should allow for comparison constraint with _', function () {
		const result = parse('a[custom_op]=b');
		result.should.eql({type: parser.NODE_TYPE.CONSTRAINT, selector: 'a', comparison: 'custom_op', argument: 'b'});
	});
	...
	it('should allow standard not-equal-operator', function () {
		const result = parse('a[ne]=b');
		result.should.eql({type: parser.NODE_TYPE.CONSTRAINT, selector: 'a', comparison: 'ne', argument: 'b'});
	});
